...
export function ConciliacionClient() {
  ...
  async function cargarReportes() {
    setLoadingList(true);
    try {
      const r = await fetch("/api/contabilidad/conciliacion");
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("/api/contabilidad/conciliacion", { method: "POST", body: fd });
      const json = await res.json();
      if (!res.ok) {
        setError(json.error || "Error al procesar");
        setLoading(false);
      } else {
        router.push(`/contabilidad/conciliacion/${json.id}`);
      }
    } catch (err) {
  ...
  async function handleDelete(id) {
    setDeleting(id);
    try {
      const res = await fetch(`/api/contabilidad/conciliacion/${id}`, { method: "DELETE" });
      if (!res.ok) {
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-black/[0.06] p-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            className="px-5 py-2 rounded-lg bg-gradient-to-br from-[#065F46] to-[#059669] text-white text-sm font-medium hover:shadow-lg hover:shadow-[#065F46]/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
      ...
      <div className="bg-white rounded-xl border border-black/[0.06] overflow-hidden">
        <div className="px-5 py-3 bg-gradient-to-r from-[#065F46] to-[#059669] text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold">Conciliaciones generadas</h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#F8F9FA] text-[#595959]">
                <tr>
                  ...
                  <th className="text-center px-4 py-2 font-medium">Cobradas</th>
                  <th className="text-center px-4 py-2 font-medium">Pendientes</th>
                  <th className="text-center px-4 py-2 font-medium">MP sin GBP</th>
                  ...
                  <tr key={r.id} className={i % 2 === 1 ? "bg-[#F8F9FA]" : "bg-white"}>
                    ...
                    <td className="px-4 py-2 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-3">
                        <Link
                          href={`/contabilidad/conciliacion/${r.id}`}
